...
func handleToolboxIPScanRun(c *gin.Context, app *appctx.ServerApp) {
	...
	started := time.Now().In(core.BeijingLocation())
	run := ipScanRun{
		ID:          uuid.NewString(),
	...
	run.EndedAt = time.Now().In(core.BeijingLocation()).Format(time.RFC3339Nano)

	if err := appendIPScanHistory(app, run); err != nil {
		core.RespondAPIError500(c, "保存历史失败: "+err.Error())
		return
	}
