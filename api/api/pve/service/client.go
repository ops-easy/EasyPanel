...
func newPVEAPIClient(target pvemodel.Target, tokenPlain string) (*pveAPIClient, error) {
	base, err := normalizePVEBaseURL(target.BaseURL)
	if err != nil {
		return nil, err
	}
	target.BaseURL = base
	tr := &http.Transport{}
	if target.SkipTLS {
		tr.TLSClientConfig = &tls.Config{InsecureSkipVerify: true} //nolint:gosec // 内网自签证书由用户显式开启。
	}
	return &pveAPIClient{
		target:     target,
...
func (c *pveAPIClient) apiURL(path string, query url.Values) string {
...
func (c *pveAPIClient) do(ctx context.Context, method, path string, query url.Values, body any) (json.RawMessage, error) {
	...
	req.Header.Set("Authorization", buildPVEAuthHeader(c.target.TokenID, c.tokenPlain))
	if body != nil {
