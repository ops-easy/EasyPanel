...
//
// 生产入口已经迁移到 backend/router/router.go；这里保留给旧测试和迁移期兼容，
// 新模块路由不要继续挂到本函数里。
...
// 生产路由聚合以 backend/router/router.go 为准。
func attachLegacyRoutesForCoreRouter(r *gin.Engine, app *ServerApp) *gin.RouterGroup {
	...
	api.Use(apiResponseCacheMiddleware(app))
	{
		...
		api.POST("/prometheus/source", func(c *gin.Context) { handlePrometheusSource(c, app) })
...
// resolveFrontendDistDir 查找 Vite 构建产物：环境变量 DASHBOARD_STATIC_DIR、当前目录、可执行文件旁 frontend/dist。
...
func registerFrontendRoutes(r *gin.Engine, app *ServerApp) {
	...
	log.Printf("未找到 %s，前端页面不可用；请执行: cd frontend && npm run build", frontendIndex)
	r.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusServiceUnavailable, gin.H{"error": "未找到前端构建产物，请执行: cd frontend && npm run build"})
	})
	r.NoRoute(func(c *gin.Context) {
		if strings.HasPrefix(c.Request.URL.Path, "/api/") {
			c.JSON(http.StatusNotFound, gin.H{"error": "API 不存在或未注册，请确认已部署包含该接口的版本"})
...
func buildConfigMapResponse(app *ServerApp, role string, eff *EffectiveDashboardPermissions) gin.H {
	...
	out := gin.H{
		...
		"ingressNginxNamespace":          effectiveIngressNginxNamespace(app.Runtime()),
		...
		"kubePrometheusStackNamespace":   effectiveKubePrometheusStackNamespace(app.Runtime()),
		"kubePrometheusStackReleaseName": effectiveKubePrometheusStackReleaseName(app.Runtime()),
		"victoriaLogsNamespace":          effectiveVictoriaLogsNamespace(app.Runtime()),
		"victoriaLogsReleaseName":        effectiveVictoriaLogsReleaseName(app.Runtime()),
		"metricsServerNamespace":         effectiveMetricsServerNamespace(app.Runtime()),
		"kubernetesDashboardNamespace":   effectiveKubernetesDashboardNamespace(app.Runtime()),
		"kubernetesDashboardReleaseName": effectiveKubernetesDashboardReleaseName(app.Runtime()),
