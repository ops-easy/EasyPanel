...
func appRedisTemplateListFromMySQL(ctx context.Context, db *sql.DB) ([]appRedisTemplateRow, error) {
	...
	rows, err := db.QueryContext(ctx,
		`SELECT id, name, description, config_json, created_at, updated_at, created_by FROM easypanel_app_redis_templates ORDER BY id DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func appRedisTemplateGetByID(ctx context.Context, db *sql.DB, id int64) (*appRedisTemplateRow, error) {
	...
	err := db.QueryRowContext(ctx,
		`SELECT id, name, description, config_json, created_at, updated_at, created_by FROM easypanel_app_redis_templates WHERE id=?`,
		id,
...
func appRedisTemplateDelete(ctx context.Context, db *sql.DB, id int64) error {
	_, err := db.ExecContext(ctx, `DELETE FROM easypanel_app_redis_templates WHERE id=?`, id)
	return err
}

func appRedisTemplateInsert(ctx context.Context, db *sql.DB, name, description, configJSON, createdBy string) (int64, error) {
	res, err := db.ExecContext(ctx,
		`INSERT INTO easypanel_app_redis_templates (name, description, config_json, created_by) VALUES (?,?,?,?)`,
...
func appRedisTemplateUpdate(ctx context.Context, db *sql.DB, id int64, name, description, configJSON string) error {
	_, err := db.ExecContext(ctx,
		`UPDATE easypanel_app_redis_templates SET name=?, description=?, config_json=? WHERE id=?`,
