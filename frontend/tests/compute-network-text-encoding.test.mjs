...
function collectSourceFiles(entry) {
  const abs = resolve(frontendRoot, entry);
  ...
  return readdirSync(abs, { withFileTypes: true }).flatMap((dirent) => {
    ...
    return dirent.isDirectory() ? collectSourceFiles(relative(frontendRoot, child)) : collectSourceFiles(relative(frontendRoot, child));
  });
}
...
test("frontend user-facing sources stay valid UTF-8 Chinese", () => {
  ...
  for (const file of files) {
    ...
    if (issues.length > 0) {
      failures.push(`${relative(frontendRoot, file)}: ${issues.join(", ")}`);
    }
  }
