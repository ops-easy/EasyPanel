...
const CLOUD_VM_BOOTSTRAP_NAV_STATE = { allowIncompleteBootstrap: true };
...
export default function AppCenterCloudVmBootstrap() {
  ...
  const [ns, setNs] = useState("easypanel-cloud-vm");
  ...
  useEffect(() => {
    ...
    setNs(bootQ.data.defaultNamespace || "easypanel-cloud-vm");
  ...
  const onTabChange = (value: string) => {
    if (value === "create") {
      navigate(CLOUD_VM_CREATE_PATH, { state: CLOUD_VM_BOOTSTRAP_NAV_STATE });
      return;
    }
    if (value === "list") {
      navigate(CLOUD_VM_LIST_PATH, { state: { ...CLOUD_VM_BOOTSTRAP_NAV_STATE, mainTab: "list" } });
      return;
    }
    navigate(CLOUD_VM_BOOTSTRAP_PATH);
  };
