    ...
    <div className={styles.container}>
      <div className={styles.header}>
        <div className={styles.titleRow}>
          <div>
            <h2 className={styles.title}>{mode === "guides" ? "页面指南" : "Markdown 文档库"}</h2>
            <p className={styles.sub}>
              {mode === "guides" ? "系统标识 · 路由联动 · 右下角帮助" : ".md · GFM · 代码高亮 · Mermaid"}
            </p>
          </div>
      ...
      <div className={styles.list}>
        {documents.length === 0 ? (
          <div className={styles.empty}>
            ...
            <p className="font-medium text-slate-600">{mode === "guides" ? "暂无页面指南" : "文库为空"}</p>
            <p className={styles.emptyTip}>
              {mode === "guides"
                ? "页面指南由系统标识绑定到路由，默认种子会在后端初始化后出现。"
                : "使用顶部「Markdown」或「画布」创建文档，支持版本与飞书风分享页。"}
            </p>
          </div>
        ) : filtered.length === 0 ? (
          ...
          filtered.map((doc) => (
            ...
            >
              ...
              <div className={styles.itemContent}>
                ...
                {doc.guide ? (
                  <div className="mt-1 flex min-w-0 flex-wrap gap-1 text-[10px] leading-tight text-violet-700">
                    <span className="rounded bg-violet-50 px-1.5 py-0.5">{doc.guide.guideKey}</span>
                    <span className="max-w-full truncate rounded bg-slate-100 px-1.5 py-0.5 text-slate-600">
                      {doc.guide.matchType}:{doc.guide.routePattern}
                    </span>
                    {!doc.guide.enabled ? (
                      <span className="rounded bg-amber-50 px-1.5 py-0.5 text-amber-700">停用</span>
                    ) : null}
                  </div>
                ) : null}
              </div>
